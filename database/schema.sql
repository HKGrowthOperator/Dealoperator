...
CREATE TABLE participants(id text PRIMARY KEY,import_key text UNIQUE,name text NOT NULL,company text NOT NULL DEFAULT '',role text NOT NULL DEFAULT '',email text,owner text UNIQUE,public_consent boolean NOT NULL DEFAULT false,claimed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now());
